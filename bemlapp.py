        ...
        if len(order_quantity)>=1 and len(stbi)>=1:
            ...
            for index1,row1 in schedules_new.iterrows():
                if paid<row1['Scheduled Qty']: 
                    ...
                    datestring=row1['Deliv. Date'].isoformat()
                    if datestring[0:10]!='2222-02-02':
                        gr_pending.loc[index,'Result_date']= datestring[0:10]
                    else:
                        gr_pending.loc[index,'Result_date']='unscheduled'
